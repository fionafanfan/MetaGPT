...
from mygpts.action_write_code import MyWriteCode
from mygpts.action_write_code_review import MyWriteCodeReview
...
class MyEngineer(Role):
    ...
    def __init__(
        self,
        name: str = "埃勒克斯",
        profile: str = "是一名开发工程师",
        goal: str = "负责编写并可能审查代码。",
        constraints: str = "代码需要遵守PEPE8规范",
        n_borg: int = 1,
        use_code_review: bool = False,
    ) -> None:
        """Initializes the Engineer role with given attributes."""
        super().__init__(name, profile, goal, constraints, is_human=True)
        self._init_actions([MyWriteCode])
        self.use_code_review = use_code_review
        if self.use_code_review:
            self._init_actions([MyWriteCode, MyWriteCodeReview])
    ...
    def get_workspace(self) -> Path:
        ...
        # Codes are written in workspace_bak2/{package_name}/{package_name}
    ...
    async def _act_sp(self) -> Message:
        ...
        for todo in self.todos:
            code = await MyWriteCode().run(context=self._rc.history, filename=todo)
            # logger.info(todo)
            # logger.info(code_rsp)
    ...
    async def _act_sp_precision(self) -> Message:
        ...
        for todo in self.todos:
            ...
            context_str = "\n".join(context)
            # Write code
            code = await MyWriteCode().run(context=context_str, filename=todo)
            # Code review
            if self.use_code_review:
                try:
                    rewrite_code = await MyWriteCodeReview().run(context=context_str, code=code, filename=todo)
                    code = rewrite_code
